...
@EFragment(R.layout.fragment_view_pager_check_list)
public class ViewPagerCheckListFragment extends BaseFragment {

    private static String TAG = ViewPagerCheckListFragment.class.getName();

    public static final int CATE_DAU_HIEU_NGHI_NGAI = 8;
    public static final String RATE_ID_DAU_HIEU_NGHI_NGAI = "14";
    ...
    public ViewPagerCheckListFragment() {
        mQuestionIdSelected = new ArrayList<>();
    }

    public static ViewPagerCheckListFragment newInstance(Kid level, Category category) {
        ViewPagerCheckListFragment_ fragment = new ViewPagerCheckListFragment_();
    ...
    private void updateAnswerCheckList(final Question questionSelected) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                String questionsId = TextUtils.join(",", mQuestionIdSelected);
                if (TextUtils.isEmpty(questionsId) && mCategory.getId() == CATE_DAU_HIEU_NGHI_NGAI) {
                    rateId = RATE_ID_DAU_HIEU_NGHI_NGAI;
                }

                ALog.e(TAG, "RateId:" + rateId + ":::" + " QuestionsId:" + questionsId + ":::" + " Score:" + mScore);
                saveScoreToBean(mScore, rateId, questionsId);
            }
        }).run();
    }

    public boolean isNeedChooseAnswer(){
        return mQuestionIdSelected.size() == 0 && mCategory.getId() != CATE_DAU_HIEU_NGHI_NGAI;
    }

    public void selectDefaultDauHieuNghiNgai() {
        if (mQuestionIdSelected.size() == 0 && mCategory.getId() == CATE_DAU_HIEU_NGHI_NGAI) {
            int score = 0;
            String rateId = RATE_ID_DAU_HIEU_NGHI_NGAI;
            String questionId = "";
            saveScoreToBean(score, rateId, questionId);
        }
    }

    private void saveScoreToBean(int score, String rateId, String questionIds) {
        // Save to bean
        QuestionScore mQuestionScore = new QuestionScore();
        mQuestionScore.setLevel(mKid);
        mQuestionScore.setCategory(mCategory);
        mQuestionScore.setScore(score);
        mQuestionScore.setTotalScore(mQuestions.size());
        mQuestionScore.setRateId(rateId);
        mQuestionScore.setQuestionIds(questionIds);
        KidBean.getInstance().addScore(mQuestionScore);

        for (QuestionScore qc : KidBean.getInstance().getQuestionScores()){
            ALog.e("QuestionScoresResults", qc.toString());
        }
    }
